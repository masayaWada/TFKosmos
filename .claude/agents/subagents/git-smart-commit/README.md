...
エージェントの動作を検証するための自動テストスクリプトが用意されています。
...
# クイックモード（基本テストのみ）
.claude/agents/subagents/git-smart-commit/test.sh -q

# 詳細ログ付き
.claude/agents/subagents/git-smart-commit/test.sh -v

# ヘルプ表示
.claude/agents/subagents/git-smart-commit/test.sh -h
```

#### テスト対象

自動テストスクリプトは以下のテストケースを実行します：

- **TC3**: 機密ファイルが含まれる場合の検出
- **TC4**: ステージング済み変更がない場合の処理
- **TC5**: main ブランチへのコミット警告
- **TC8**: 大量ファイル変更の警告
- **TC9**: Git リポジトリでない場合のエラー
- **TC11**: マージコミットの検出
- **TC13**: Issue番号の自動リンク

#### CI/CD統合

GitHub Actions で自動テストが実行されます：

- **トリガー**: `.claude/agents/**` の変更時
- **ワークフロー**: `.github/workflows/agent-tests.yml`
- **実行環境**: Ubuntu Latest

エージェント関連のファイルを変更してPRを作成すると、自動的にテストが実行されます。
